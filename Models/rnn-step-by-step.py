    ...
    def train(self, data_loader: DataLoader, epochs=30):
        for epoch in range(epochs):
            ...
            for batch in data_loader:
                ...
                num_samples_in_batch, s, v = batch.shape
                batch_loss = 0

                # For each sample in the batch:
                for sample in batch:
                    for end_of_seq in range(1, math.ceil(len(sample)/8) - 1):
                ...
                print(f"Batch {batch} - Average Loss: {batch_loss / (grad_updates % num_samples_in_batch)}")
